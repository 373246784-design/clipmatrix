...
def _extract_key_insight(aid: str, insights_data: dict) -> dict:
    ...
    if info.get('status') != 'COMPLETED' or not info.get('insights'):
        return {}
    ...
    for net, items in content.get('organicNetworks', {}).items():
        for item in items:
            ...
            if 'pattern' in t or 'detect' in t:
                result['pattern'] = item.get('text', '')[:250]
                break
    ...
    if exec_sum and ('fail' in exec_sum.lower() or 'low' in exec_sum.lower() or
                     'negligible' in exec_sum.lower() or 'zero' in exec_sum.lower()):
        ...
        for w in winners:
            ...
            for net, items in content.get('organicNetworks', {}).items():
                for item in items:
                    ...
                    if 'recommend' in t or 'pattern' in t or 'detect' in t:
