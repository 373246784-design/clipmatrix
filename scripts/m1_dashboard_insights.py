...
def get_or_create_dashboard(token: str, uid: str, blog_id: str,
    ...
    if not force_recreate:
        ...
        for d in dashboards:
            if d.get("status") in ("SYNCHRONIZED", "DRAFT") and \
               "pandajourney" in (d.get("title", "") + d.get("description", "")).lower():
                did = d["id"]
                logger.info(f"  复用dashboard {did} for blog={blog_id}")
    ...
    logger.info(f"  检测到网络: {nets}")
    
    if not nets:
        logger.warning(f"  blog={blog_id} 无可用网络")
        return {"dashboard_id": None, "status": "NO_NETWORKS", "insights": None}
    
    # 3. 创建新dashboard — 使用检测到的实际网络
    logger.info(f"  创建dashboard for blog={blog_id} networks={nets}")
